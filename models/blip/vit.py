...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
        ...
        self.cls_head = nn.Linear(self.embed_dim, self.num_classes) if self.num_classes > 0 else nn.Identity()
    ...
    def forward_encoder(self, x, register_blk=-1):
        B, nc, w, h = x.shape
        x = self.patch_embed(x)
        cls_tokens = self.cls_token.expand(B, -1, -1)
        x = torch.cat((cls_tokens, x), dim=1)
        pos_embeddings = self.interpolate_pos_encoding(x, w, h)
        x = x + pos_embeddings[:,:x.size(1),:]
        x = self.pos_drop(x)
    ...
    def forward_encoder_asy(self, x, asy_pos_ratio, register_blk=-1):
        x_asy = x
        ...
        crop_pos_embed_with_cls = torch.cat((self.pos_embed[:, :1], crop_pos_embed), dim=1)
        x_asy = x_asy + crop_pos_embed_with_cls
        x_asy = self.pos_drop(x_asy)
        for i,blk in enumerate(self.blocks):
            x_asy = blk(x_asy, register_blk==i)
        x_asy_out = self.norm(x_asy)
        return x_out[:, 1:], x_asy_out[:, 1:]
    ...
    def forward_no_adapter(self, x, register_blk=-1):
        B, nc, w, h = x.shape
        x = self.patch_embed(x)
        cls_tokens = self.cls_token.expand(B, -1, -1)
        x = torch.cat((cls_tokens, x), dim=1)
    ...
    def forward_features_finetune(self, imgs):
        imgs = self.forward_encoder(imgs)
        latent_concat = self.adapter(imgs)  # remove [CLS] token
        return latent_concat
    ...
    def forward(self, x_lst, use_adapter=True, use_asy_pos_embedding=True, temp=0.03, asy_pos_ratio=3.0, lambda_reg=0.5, is_training=False, eval_metric=False, finetuning=False, **kwargs):
        if use_adapter:
            if is_training:
                # for classification
                if finetuning:
                    # Get original batch size and temporal dimension
                    B, C, T, H, W = x_lst.shape
                    # Reshape from [B, 3, T, 224, 224] to [B*T, 3, 224, 224]
                    samples_reshaped = x_lst.permute(0, 2, 1, 3, 4).contiguous().view(B*T, C, H, W)
                    outputs = self.forward_features_finetune(samples_reshaped)
                    _, L, D = outputs.shape
                    # Reshape from [B*T, L, D] to [B, T, L, D]
                    outputs_reshaped = outputs.view(B, T, L, D)
                    # Average over middle two dimensions (T and L) to get [B, D]
                    outputs = outputs_reshaped.mean(dim=(1, 2))
                    # Map to the nb_class dim to get [B, K]
                    ret = self.cls_head(outputs)
                else:
            ...
            if is_training:
                ...
                if finetuning:
                    ...
                    # Reshape from [B, 3, T, 224, 224] to [B*T, 3, 224, 224]
                    samples_reshaped = x_lst.permute(0, 2, 1, 3, 4).contiguous().view(B*T, C, H, W)
                    outputs = self.forward_encoder(samples_reshaped)
                    _, L, D = outputs.shape
                    # Reshape from [B*T, L, D] to [B, T, L, D]
                    outputs_reshaped = outputs.view(B, T, L, D)
                    # Average over middle two dimensions (T and L) to get [B, D]
